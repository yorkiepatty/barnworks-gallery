...
def text_to_speech(
    ...
    tts = gTTS(text=text, lang=lang, slow=effective_slow, tld=selected_voice.get("tld", "com"))
...
def get_voice_preview(voice_id: str, text: str = "Hello, this is a sample of my voice.") -> str:
    ...
    content_hash = hashlib.sha256(f"preview_{voice_id}_{text}".encode()).hexdigest()
...
# DISABLED: These routes should be in app.py, not here
# This was causing duplicate route registration errors
# app = Flask(__name__)
# 
# 
# @app.route("/")
...
# DISABLED: These Flask routes should not be here - causes conflicts with main app
...
if __name__ == "__main__":
    ...
    # app.run(debug=(os.getenv("ALPHAVOX_DEBUG", "0") == "1"))  # Disabled - no standalone app
...
__all__ = ['text_to_speech', 'text_to_speech_with_emotion', 'get_voice_preview', 'get_voice_description', 'get_all_voices', 'get_voice_for_emotion']
