...
class MemoryManager:
    ...
    def update_user_preference(self, pref_key: str, pref_value: Any):
        """Update a specific user preference and persist."""
        if "user_preferences" not in self.long_term_memory:
            self.long_term_memory["user_preferences"] = {}
            
        self.long_term_memory["user_preferences"][pref_key] = pref_value
        self.store(f"pref_{pref_key}", pref_value)
        self.save()
        print(f"🧬 Bio-Bridge: Updated long-term preference: {pref_key}")
...
def get_memory_manager(user_id: str = "default_user") -> MemoryManager:
    """
    Get or create a MemoryManager singleton for a specific user.
    """
    if user_id not in _memory_managers:
        manager = MemoryManager(memory_dir=f"./memory/{user_id}")
        manager.load()
