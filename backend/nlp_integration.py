...
class NLPIntegrationService:
    ...
    def enhance_response(
        ...
        domain_expertise_level = self.domain_expertise.get(domain, 0.75) if domain else 0.75
        ...
        if not enhanced_response or enhanced_confidence < 0.6:
            if user_input:
                enhanced_response = self.nlp_processor.generate_phd_level_response(user_input)
            else:
    ...
    def _analyze_vocabulary_complexity(self, text: str) -> float:
        ...
        complex_words = [token.text for token in doc if len(token.text) > 8 and token.is_alpha]
        ...
        complexity_score = (
            (complex_ratio * 0.4) + (academic_term_ratio * 0.4) + (sentence_length_factor * 0.2)
        )

        return complexity_score

    def get_phd_level_explanation(self, topic: str, simplify: bool = False) -> str:
        domain, _ = self.nlp_processor._detect_academic_domain(self.nlp_processor.nlp(topic))
    ...
    def _generate_simplified_explanation(self, topic: str, domain: Optional[str]) -> str:
