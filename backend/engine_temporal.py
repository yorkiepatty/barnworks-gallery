...
class TemporalNonverbalEngine:
    ...
    def _load_lstm_models(self):
        ...
        if os.path.exists(gesture_model_path):
            try:
                ...
                logger.info(f"Loaded TensorFlow gesture model from {gesture_model_path}")
            except (ImportError, Exception) as e:
                logger.error(f"Failed to load TensorFlow gesture model: {e}")
                self.models["gesture"] = None
        elif os.path.exists(gesture_pkl_path):
            try:
                with open(gesture_pkl_path, "r", encoding="utf-8") as f:
                    self.models["gesture"] = json.load(f)
        ...
        if os.path.exists(eye_model_path):
            try:
                ...
                logger.info(f"Loaded TensorFlow eye movement model from {eye_model_path}")
            except (ImportError, Exception) as e:
                logger.error(f"Failed to load TensorFlow eye movement model: {e}")
                self.models["eye_movement"] = None
        elif os.path.exists(eye_pkl_path):
            try:
                with open(eye_pkl_path, "r", encoding="utf-8") as f:
                    self.models["eye_movement"] = json.load(f)
        ...
        if os.path.exists(emotion_model_path):
            try:
                ...
                logger.info(f"Loaded TensorFlow emotion model from {emotion_model_path}")
            except (ImportError, Exception) as e:
                logger.error(f"Failed to load TensorFlow emotion model: {e}")
                self.models["emotion"] = None
        elif os.path.exists(emotion_pkl_path):
            try:
                with open(emotion_pkl_path, "r", encoding="utf-8") as f:
                    self.models["emotion"] = json.load(f)
        ...
        if os.path.exists(eye_labels_path):
            try:
                with open(eye_labels_path, "r", encoding="utf-8") as f:
    ...
    def classify_gesture_sequence(self):
        ...
        """
        if self.models["gesture"] is None or len(self.gesture_buffer) < self.sequence_length:
            return {
                "expression": "Unknown",
                "intent": "Unknown",
                "confidence": 0.0,
    ...
    def classify_eye_movement_sequence(self):
        ...
        Returns:
            Dictionary with expression, intent, confidence, and message
        """
        if self.models["eye_movement"] is None or len(self.eye_buffer) < self.sequence_length:
            return {
                "expression": "Unknown",
                "intent": "Unknown",
                "confidence": 0.0,
    ...
    def classify_emotion_sequence(self):
        ...
        Returns:
            Dictionary with expression, intent, confidence, and message
        """
        if self.models["emotion"] is None or len(self.emotion_buffer) < self.sequence_length:
            return {
                "expression": "Unknown",
                "intent": "Unknown",
                "confidence": 0.0,
    ...
    def process_multimodal_sequence(
        ...
        if hasattr(self, "learning_journey") and self.learning_journey:
            ...
            self.learning_journey.update_skill_level(f"{primary_type}_recognition", 0.05)
    ...
    def get_academic_response(self, topic, depth="advanced"):
        ...
        try:
            ...
            response = conversation_engine.generate_response(context=context, persona="academic")
...
__all__ = ['load_json_file', 'get_temporal_engine', 'TemporalNonverbalEngine']
