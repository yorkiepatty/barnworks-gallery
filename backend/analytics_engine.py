...
class AnalyticsEngine:
    ...
    def log_training_access(self, user_id, tutorial_id, completion_status=None):
        ...
        self.training_access_df = pd.concat([self.training_access_df, new_row], ignore_index=True)
    ...
    def end_session(self, session_id):
        ...
        if session_id in self.sessions_df["session_id"].values:
            idx = self.sessions_df[self.sessions_df["session_id"] == session_id].index[0]
    ...
    def get_user_stats(self, user_id):
        ...
        user_symbols = self.symbol_selections_df[self.symbol_selections_df["user_id"] == user_id]
        ...
        avg_score = (
            user_games["score"].mean() if not user_games.empty and "score" in user_games else 0
        )
    ...
    def get_progress_report(self, user_id, days=30):
        ...
        if not user_interactions.empty and "timestamp" in user_interactions:
            user_interactions["date"] = pd.to_datetime(user_interactions["timestamp"]).dt.date
            daily_interactions = user_interactions.groupby("date").size().reset_index(name="count")
        ...
        if not user_symbols.empty and "timestamp" in user_symbols:
            ...
            daily_symbols = user_symbols.groupby("date").size().reset_index(name="count")
        ...
        if not user_games.empty and "timestamp" in user_games and "score" in user_games:
            ...
            daily_scores = user_games.groupby("date")["score"].mean().reset_index(name="avg_score")
        ...
        daily_interactions_list = (
            daily_interactions.to_dict("records") if not daily_interactions.empty else []
        )
        daily_symbols_list = daily_symbols.to_dict("records") if not daily_symbols.empty else []
        daily_scores_list = daily_scores.to_dict("records") if not daily_scores.empty else []
        ...
        if not user_interactions.empty and "timestamp" in user_interactions:
            ...
            if len(user_interactions) >= 10:
                ...
                if second_rate > first_rate * 1.2:  # 20% increase
                    insights["progress_indicators"].append("Increasing interaction frequency")
                elif second_rate < first_rate * 0.8:  # 20% decrease
                    insights["challenge_areas"].append("Decreasing interaction frequency")
