...
def text_to_speech(
    ...
    logger.info(f"Using voice: {selected_voice['label']} (TLD: {selected_voice.get('tld', 'com')})")
    ...
    # --- BIOLOGICAL AUDIT: PHASE 3 ADAPTIVE PACING (EMPATHY RESPONSE) ---
    try:
        from memory_manager import get_memory_manager
        memory = get_memory_manager() # Uses default_user profile for now
        prefs = memory.long_term_memory.get("user_preferences", {})
        pacing_weights = prefs.get("cognitive_pacing", {})
        processing_patience = pacing_weights.get("processing_patience", 0.5)
        
        if processing_patience > 0.8:
            logger.info(f"🧬 EMPATHY RESPONSE TRIGGERED: Patience weight high ({processing_patience:.2f}). Enforcing slow TTS pacing.")
            effective_slow = True
    except Exception as e:
        logger.warning(f"Could not read LTP weights for TTS pacing: {e}")
    # ---------------------------------------------------------------------
...
__all__ = ['text_to_speech', 'get_voice_preview', 'get_voice_description']
