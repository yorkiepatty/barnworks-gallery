...
class VoiceSynthesizer:
    ...
    def set_defaults(self, language=None, accent=None, slow=None):
        ...
        if accent:
            if accent in self.english_accents:
                ...
                logger.warning(f"Accent '{accent}' not available, using current default")
    ...
    def speak_with_emotion(
        ...
        if emotion:
            logger.info(f"Applying emotional context: {emotion}, intensity: {emotion_intensity}")
...
__all__ = ['get_voice_synthesizer', 'VoiceSynthesizer']
