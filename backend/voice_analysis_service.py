...
class VoiceAnalysisService:
    ...
    def analyze_tone(self, audio_data: bytes) -> Tuple[Dict[str, float], Dict[str, float], float]:
        ...
        try:
            ...
            intensity_variations = float(np.std(max_frame_values)) if max_frame_values else 0.0

            # Enhanced emotional state analysis
            emotions = {
                "calm": max(
                    min(
                        (1 - min(variance, 1.0)) * 0.6 + (1 - min(intensity_variations, 1.0)) * 0.4,
                        1.0,
                    ),
                    0.0,
                ),
                "engaged": max(min(mean_amplitude * 0.4 + pattern_strength * 0.6, 1.0), 0.0),
                "uncertain": max(
                ...
                "overwhelmed": max(min(variance * 0.6 + intensity_variations * 0.4, 1.0), 0.0),
            }
            ...
            communication_patterns = {
                ...
                "engagement_level": max(min(mean_amplitude + pattern_strength * 0.5, 1.0), 0.0),
            }
